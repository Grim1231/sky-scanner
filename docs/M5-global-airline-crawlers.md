...
### E2E 테스트 결과: **25 passed, 1 xfailed, 0 failed**
...
| 5 | 에어프레미아 | YP | L2+L3 | primp L2 기본, Playwright CF bypass fallback | ⚠️ xfail (CF 간헐차단) |
| 6 | 에어서울 | RS | L2+L3 | primp → Playwright CF bypass fallback | ⚠️ xfail (CF WAF `/I/KO/` 차단) + Amadeus fallback ✅ |
...
### M5 글로벌 항공사 — L2 완료 (17개, NH/TG/SQ Sputnik 전환 포함)
...
| 27 | SQ via Amadeus | SQ | SIN | Amadeus GDS | Amadeus fallback (코드셰어) | ✅ |

### L3 Playwright — 차단됨 (2개)
| # | 항공사 | 코드 | 허브 | L3 전략 | 차단 이유 | 대안 |
|---|--------|------|------|---------|-----------|------|
| 28 | Air France-KLM | AF/KL | CDG/AMS | 폼 → GraphQL intercept | Akamai HTTP/2 차단 | Amadeus |
| 29 | Qatar Airways | QR | DOH | 폼 + 딥링크 URL | Akamai WAF 403 | Amadeus |
...
| (미커밋) | 02-16 | NH/TG/SQ Sputnik 전환, TK L3 Playwright, RS L3 인프라, E2E 테스트 | ~30 |
...
| **EveryMundo Sputnik (공유 키)** | Cloudflare | **NZ, ET, JL, NH, TG, SQ** (6개) |
...
| Playwright 폼 자동화 (차단됨) | Akamai | ❌ AF/KL, QR |

---

## 커버리지 요약

- **총 크롤러**: 29개 (L1 2 + Korean LCC 7 + GDS 1 + Global L2 17 + Global L3 차단 2)
- **E2E 테스트 통과**: 25/26 (Kiwi xfail)
- **Sputnik 사용 항공사**: 6개 (NZ, ET, JL, NH, TG, SQ)
- **Amadeus fallback**: AF/KL, QR + GA, SV, EY, MU, CZ, DL, AA, UA, QF, AC (12개)
- **한국 출발 주요 허브 커버리지**:
  IST ✅ DOH(Amadeus) SIN ✅ HKG ✅ NRT ✅ FRA ✅ CDG(Amadeus) AMS(Amadeus) BKK ✅ TPE ✅ WAW ✅ KUL ✅ ADD ✅ DXB ✅
