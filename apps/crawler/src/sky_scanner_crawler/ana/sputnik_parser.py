"""Parse ANA Sputnik fare search response into NormalizedFlight objects."""
...
def parse_fares(
    ...
    *,
    ...
    """Convert ANA Sputnik fare entries into NormalizedFlight list.
    ...
    for entry in raw:
        ...
        try:
            ...
            logger.warning("Invalid date in ANA fare: %s", date_str)
            continue
        ...
        flights.append(
            NormalizedFlight(
                flight_number=f"{_NH_CODE}-{dep_airport}{arr_airport}",
                airline_code=_NH_CODE,
                airline_name=_NH_NAME,
                operator=_NH_CODE,
                origin=dep_airport,
    ...
    logger.info(
        "Parsed %d daily lowest fares for %s->%s from All Nippon Airways",
        len(flights),
        origin_filter or "*",
