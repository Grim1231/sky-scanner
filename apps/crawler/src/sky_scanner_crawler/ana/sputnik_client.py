"""HTTP client for ANA's fare search via EveryMundo Sputnik API.

ANA (All Nippon Airways) uses EveryMundo's airTrfx platform to publish
lowest fares at ``flights.ana.co.jp``.  The ``airfare-sputnik-service``
exposes a fare search endpoint that returns daily lowest one-way fares
across the entire NH route network.

Flow:
1. POST ``openair-california.airtrfx.com/airfare-sputnik-service/v3/nh/fares/search``
...
3. Include ``Referer`` and ``Origin`` headers pointing to the ANA flights page
...
_FARE_SEARCH_URL = (
    "https://openair-california.airtrfx.com/airfare-sputnik-service/v3/nh/fares/search"
)
_REFERER = "https://flights.ana.co.jp/en-us/"
_ORIGIN_HEADER = "https://flights.ana.co.jp"
...
class AnaSputnikClient:
    """Async client for ANA fare search via EveryMundo Sputnik."""
    ...
    def _fetch_fares(
        ...
        *,
        ...
        if resp.status_code != 200:
            msg = f"ANA fare search: HTTP {resp.status_code}"
    ...
    async def search_fares(
        ...
        *,
        ...
        """Fetch daily lowest fares for ANA routes.
        ...
        destination:
            IATA airport code to filter destination (e.g. ``SIN``).
        ...
        logger.debug(
            "ANA fares %s->%s: %d entries, %d with prices",
            origin or "*",
            destination or "*",
            len(result),
            priced,
    ...
    async def health_check(self) -> bool:
        """Check if the ANA fare API is accessible."""
        try:
