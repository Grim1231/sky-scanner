"""Parse Air New Zealand Sputnik fare search response into NormalizedFlight objects."""
...
_FARE_CLASS_MAP: dict[str, CabinClass] = {
    ...
    "BUSINESSPREMIER": CabinClass.BUSINESS,
...
def parse_fares(
    ...
    *,
    ...
    """Convert Air New Zealand Sputnik fare entries into NormalizedFlight list.
    ...
    Each entry in the raw list looks like::

        {
            ...
            "journeyType": "ONE_WAY",
            "outboundFlight": {
                "departureAirportIataCode": "AKL",
                "arrivalAirportIataCode": "NRT",
                "fareClass": "ECONOMY",
                "fareClassInput": "seat",
                ...
            },
    ...
    for entry in raw:
        ...
        currency: str = price_spec.get("currencyCode", "NZD")
        ...
        try:
            ...
            logger.warning("Invalid date in Air NZ fare: %s", date_str)
            continue
        ...
        flights.append(
            NormalizedFlight(
                flight_number=f"{_AIR_NZ_CODE}-{dep_airport}{arr_airport}",
                airline_code=_AIR_NZ_CODE,
                airline_name=_AIR_NZ_NAME,
                operator=_AIR_NZ_CODE,
                origin=dep_airport,
    ...
    logger.info(
        "Parsed %d daily lowest fares for %s->%s from Air New Zealand",
        len(flights),
        origin_filter or "*",
