...
from .client import EthiopianAirlinesClient
from .response_parser import parse_fares

logger = logging.getLogger(__name__)


class EthiopianAirlinesCrawler(BaseCrawler):
    """L2 crawler: Ethiopian Airlines daily lowest fares via Sputnik API.

    Ethiopian Airlines publishes fares through EveryMundo's airTrfx platform.
    The Sputnik fare search endpoint returns up to ~300 days of daily
    lowest one-way fares across the entire ET route network.
    ...
    def __init__(self) -> None:
        self._client = EthiopianAirlinesClient(timeout=settings.l2_timeout)
    ...
    async def crawl(self, task: CrawlTask) -> CrawlResult:
        ...
        try:
            ...
            elapsed_ms = int((time.monotonic() - start) * 1000)
            logger.exception("Ethiopian Airlines crawl failed")
            return CrawlResult(
                source=DataSource.DIRECT_CRAWL,
                crawled_at=datetime.now(tz=UTC),
                duration_ms=elapsed_ms,
    ...
    async def health_check(self) -> bool:
        """Check if the Ethiopian Airlines fare API is reachable."""
