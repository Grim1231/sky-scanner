"""Parse SQ Sputnik fare search response into NormalizedFlight objects."""
...
def parse_fares(
    ...
    *,
    ...
    """Convert Singapore Airlines Sputnik fare entries into NormalizedFlight list.
    ...
    Each entry in the raw list looks like::

        {
            ...
            "searchDate": "2026-02-16T01:10:30.000+0000",
            ...
        }
    ...
    for entry in raw:
        ...
        try:
            ...
            logger.warning("Invalid date in SQ fare: %s", date_str)
            continue
        ...
        flights.append(
            NormalizedFlight(
                flight_number=f"{_SQ_CODE}-{dep_airport}{arr_airport}",
                airline_code=_SQ_CODE,
                airline_name=_SQ_NAME,
                operator=_SQ_CODE,
                origin=dep_airport,
    ...
    logger.info(
        "Parsed %d daily lowest fares for %s->%s from Singapore Airlines",
        len(flights),
        origin_filter or "*",
