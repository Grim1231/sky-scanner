"""HTTP client for Singapore Airlines fare search via EveryMundo Sputnik API.

Singapore Airlines uses EveryMundo's airTrfx platform to publish lowest fares
at ``www.singaporeair.com``.  The ``airfare-sputnik-service`` exposes a fare
search endpoint that returns daily lowest one-way fares across the entire SQ
route network.

Flow:
1. POST ``openair-california.airtrfx.com/airfare-sputnik-service/v3/sq/fares/search``
...
3. Include ``Referer`` and ``Origin`` headers pointing to the SQ website
...
_FARE_SEARCH_URL = (
    "https://openair-california.airtrfx.com/airfare-sputnik-service/v3/sq/fares/search"
)
_REFERER = "https://www.singaporeair.com/"
_ORIGIN_HEADER = "https://www.singaporeair.com"
...
class SqSputnikClient:
    """Async client for Singapore Airlines fare search via EveryMundo Sputnik."""
    ...
    def _fetch_fares(
        ...
        *,
        ...
        if resp.status_code != 200:
            msg = f"SQ fare search: HTTP {resp.status_code}"
    ...
    async def search_fares(
        ...
        *,
        ...
        """Fetch daily lowest fares for Singapore Airlines routes.

        Parameters
        ----------
        origin:
            IATA airport code to filter origin (e.g. ``SIN``).
        ...
        logger.debug(
            "SQ fares %s->%s: %d entries, %d with prices",
            origin or "*",
            destination or "*",
            len(result),
            priced,
    ...
    async def health_check(self) -> bool:
        """Check if the SQ fare API is accessible."""
        try:
