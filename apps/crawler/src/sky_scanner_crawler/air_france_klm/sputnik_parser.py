"""Parse Air France-KLM Sputnik fare search response into NormalizedFlight objects.

The Sputnik API returns fare entries with the following structure::

    {
        ...
        "searchDate": "2026-02-14T16:57:35.962+0000",
        ...
    }

Each fare entry represents a daily lowest fare for a given route and
cabin class.  There are no flight numbers or times -- only departure
dates and lowest prices.
"""
...
_FARE_CLASS_MAP: dict[str, CabinClass] = {
    ...
    "PREMIUM COMFORT": CabinClass.PREMIUM_ECONOMY,
    "BUSINESS": CabinClass.BUSINESS,
    "FIRST": CabinClass.FIRST,
    "LA PREMIERE": CabinClass.FIRST,
}
...
def parse_sputnik_fares(
    ...
    *,
    ...
    """Convert AF-KLM Sputnik fare entries into NormalizedFlight list.
    ...
    for entry in raw:
        ...
        # Determine the airline code from the entry or parameter.
        entry_airline = entry.get("airline", {}).get("iataCode", airline_code)
        resolved_airline = entry_airline.upper() if entry_airline else airline_code
        resolved_name = AIRLINE_NAMES.get(resolved_airline, airline_name)
        ...
        try:
            ...
            logger.warning("Invalid date in %s fare: %s", resolved_airline, date_str)
            continue
        ...
        flights.append(
            NormalizedFlight(
                flight_number=f"{resolved_airline}-{dep_airport}{arr_airport}",
                airline_code=resolved_airline,
                airline_name=resolved_name,
                operator=resolved_airline,
                origin=dep_airport,
    ...
    logger.info(
        "Parsed %d daily lowest fares for %s->%s from %s",
        len(flights),
        origin_filter or "*",
